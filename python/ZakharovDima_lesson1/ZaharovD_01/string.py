...
def donuts(count):      # 2016.03.18_09:46:50 checked. prusanov
    if count >= 10:
        return 'Количество пончиков: много'
    else:
        return ''.join(('Количество пончиков: ' + str(count))) #почему на конструкцию ''.join('Количество пончиков: ',str(count)) возращается ошибка TypeError: join() takes exactly one argument (2 given)
        
        # join принимает один аргумент - список строк. Т.е., чтобы все работало нормально нужно указать доп. скобки:  ''.join(('Количество пончиков: ',str(count)))
...
def both_ends(s):       # 2016.03.18_09:46:57 checked. prusanov
    if len(s)<2:        # не забываем пробелы вокруг оператора <  (PEP-8)
        return ''
    else:
...
def fix_start(s):       # 2016.03.18_09:47:33 checked. prusanov
...
def mix_up(a, b):       # 2016.03.18_09:47:39 checked. prusanov
...
# Этот ужин хорош!
def not_bad(s):     # 2016.03.18_09:47:48 checked. prusanov
    if -1 < s.find('не') < s.find('плох') and s.find('не'):     #  достаточно будет условия   -1 < s.find('не') < s.find('плох')
        return s[:s.find('не')] + 'хорош!'
    else:
        return s
