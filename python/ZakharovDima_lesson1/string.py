...
def donuts(count):
    if count >= 10:
        ...
        return 'Количество пончиков: ' + str(count) #почему на конструкцию ''.join('Количество пончиков: ',str(count)) возращается ошибка TypeError: join() takes exactly one argument (2 given)
...
# Этот ужин хорош!
def not_bad(s):
    if -1 < s.find('не') < s.find('плох') and s.find('не'):
